function [prefDir param]=Chap17_RasterPlot(neuronNum)
...
%         centerTime=go(indDir(j)); %to center on start of movement time
        ...
        spikeCount(i)=spikeCount(i)+sum(allTimes>-0.5&allTimes<1); %pick 2 seconds around center time
    end  
    %divide by the number of trials & bin size (2 s) for a mean firing rate
    spikeCount(i)=spikeCount(i)/numTrials(i)/1.5;
end
...
corrcoef(myfun(param,ang),spikeCount)
